<?php
class latest_stories extends db2
{
	function recent_activities()
	{
		$sql1 = "SELECT *, UNIX_TIMESTAMP(event_on) as event_on
			FROM  post
			ORDER BY post_added_on DESC
				Limit 0, 4";
		...
		foreach($data as $a)
		{
			...
			if(strpos($post_description, ".", 100) > 0)
			{
					$post_description = substr($post_description, 0, strpos($post_description, ".", 100));
			}
			$post_description = '';
			$temp .= '<div class="col-3 col-sm-12">
				<img
					class="card-img-top"
					src="'.$a['post_image_upload'].'"
					width="252"
					height="168"
					alt="'.$a["post_name"].'"
				/>
				<div class="card-body">
					<p class="card-text cText">'.date('j F Y ', $a["event_on"]).'</p>
					<h5 class="card-title card-changes">
						'.$a["post_name"].'
					</h5>
					<p class="card-text" style="color: #828282; font-size: 14px; display:none">
						'.$post_description.'
					</p>
					<a href="'.to_blog_url($a["post_name"]).'" class="btn btn-success hover mt-25">View </a>
				</div>
			</div>';
		}
		return $temp;
...
<br />';
	...
	function detail()
	{
		$id = (int)$_GET["post_id"];
		// debug_r(BLOG_URL);
		if(!$id) redirect(BLOG_URL);
        $a = $this->result("SELECT post_id, `post_name`, event_timing, event_on, `post_image_upload`, `post_description`, `post_category`, UNIX_TIMESTAMP(`post_added_on`) as post_added_on, gallery_link  FROM `post` WHERE post_id = $id", 1);
        if(!$a) redirect(BLOG_URL);
		$posted_at = json_decode($a['post_category']);
		$posted_at = explode('___', $posted_at[0]);
		$category_name = $posted_at[1];
		$posted_at = $a["event_on"];
		$a["posted_at"] = $posted_at;
		$a["category_name"] = $category_name;
// debug_r($post);
		$temp = $a['post_description'];
		if($a['gallery_link'] != '')
		{
			$temp .= '<div class="spacer" style="float:none; display:block; height: 10px;"></div>';
			$temp .= '<a href="'.$a['gallery_link'].'" target="_blank" class="btn btn-success hover">View Gallery</a>';
		}
		$data_n = $a;
    $data_n["html_title"] = $a["post_name"].' '.$title.'';
		$data_n["html_heading"] = $a["post_name"].' '.$title;
	...
	{
		//not being used 
		$result = $this->result("SELECT post_id, `post_name`, event_timing, event_on, `post_image_upload`, `post_description`, `post_category`, UNIX_TIMESTAMP(`post_added_on`) as post_added_on  FROM `post` ORDER BY post_added_on DESC LIMIT 0 , 3");
		// debug_r($result);
		foreach($result as $a)
		{
			$name = strip_tags(htmlspecialchars_decode1($a["post_name"]));
			$posted_at = json_decode($a['post_category']);
			$posted_at = explode('___', $posted_at[0]);
			$category_name = $posted_at[1];
			$posted_at = $a["event_on"];
			$temp .= '
				<div class="col-md-4" data-aos="fade" data-aos-duration="1000" data-aos-easing="ease-in-out"
					data-aos-mirror="true" data-aos-once="false">
					<div class="card rounded-0 text-white">
						<a href="'.to_blog_url($a['post_name']).'"><img class="card-img rounded-0" src="'.$a['post_image_upload'].'" alt="Card image"></a>
						<div class="card-img-overlay rounded-0 p-md-5">
							<div class="d-flex flex-column h-100">
								<p class="card-text font-weight-bold">'.$category_name.'</p>
								<!-- <p class="card-text">Posted at '.$posted_at.' '.date( 'j F Y ', $a['event_on']).'</p> -->
								<a href="'.to_blog_url($a['post_name']).'"class="text-light"><h5 class="card-title">'.$name.'</h5></a>
								<a href="'.to_blog_url($a['post_name']).'" class="btn btn-explore btn-secondary mt-auto text-white">Read More</a>
							</div>
						</div>
					</div>
				</div>';
